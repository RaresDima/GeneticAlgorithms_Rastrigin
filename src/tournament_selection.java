...
public class tournament_selection {
    ...
    public static void main(String[] args){
        ...
        Vector<bitset> current_tournament_group         = new Vector<>();
        Vector<bitset> population_clone                 = new Vector<>();
        ...
        for (int generation = 0; generation < 100; ++generation){
            ...
            while (selected_population.size() < 17) {
                ...
                current_tournament_group.clear();

                population_clone.clear();
                population_clone.addAll(population);

                //   SELECT k INDIVIDUALS
                for (int i = 0; i < k; ++i) {

                    selected_index = random_generator.nextInt(population_clone.size());

                    current_tournament_group.addElement(population_clone.elementAt(selected_index));

                    population_clone.remove(selected_index);
                }

                //   BEST kj ARE SELECTED FOR SURVIVAL
                for (int i = 0; i < kj; ++i) {

                    selected_population.addElement(current_tournament_group.elementAt(select_best_individual(current_tournament_group)));

                    population.remove(select_best_individual(current_tournament_group));
                    current_tournament_group.remove(select_best_individual(current_tournament_group));
                }
            }
